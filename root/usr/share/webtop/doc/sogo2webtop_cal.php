...
<?php
/**
	This script extracts events from sogo and add them to webtop.
**/
define("DEBUG",false);
	...
	while($row = mysql_fetch_row($res2)) {
		...
		$event = $events[0]; //passo eventi singoli
           	...
           	$dateTimeZone = new DateTimeZone(date_default_timezone_get());
            	...
            	if (isset($dstart)) {
                	$starttime = $dstart;
                	$event_date = gmdate("Y-m-d 00:00:00", $starttime);
                	$arrayEvent["event_date"] = $event_date;
                	$fromtime = gmdate("Y-m-d H:i:s", $starttime);
                	$arrayEvent["fromtime"] = $fromtime;
            	}
            	if (isset($dend)) {
                	$totime = $dend;
                	$totime = gmdate("Y-m-d H:i:s", $totime);
                	$arrayEvent["totime"] = $totime;
            	}
                $allday = "false";
                $arrayEvent["allday"] = $allday;
                $busy = "true";
