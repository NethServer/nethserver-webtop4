	...
	while($row = mysql_fetch_row($res2)) {
		...
		if(count($data[0]['ADR']))
			foreach($data[0]['ADR'] as $adr)
			{
				...
				$ADR[strtolower($adr['param']['TYPE'][0])]['prov'] =   $adr['value'][4][0];
		...
		if ($TEL['voice'])
            	...
            	if (isset($FirstN)) {
                	$arrayContact["firstname"] = truncateString($FirstN, 60);
            	...
            	if (isset($TEL['fax'])) {
                	$arrayContact["cfax"] = truncateString($TEL['fax'], 50);
            	}
            	if (isset($EMAIL['work'])) {
                	$arrayContact["cemail"] = truncateString($EMAIL['work'], 80);
            	}
            	if (isset($ADR['home']['street'])) {
                	$arrayContact["haddress"] = truncateString($ADR['home']['street'], 100);
            	}
            	if (isset($ADR['home']['city'])) {
            	...
            	if (isset($TEL['cell'])) {
                	$arrayContact["cmobile"] = truncateString($TEL['cell'], 50);
                	$arrayContact["hmobile"] = truncateString($TEL['cell'], 50);
            	}
            	if (isset($EMAIL['home'])) {
		...
		if(DEBUG)
		{
			echo " FirstN = ".print_r($FirstN,true)."\n";
			echo " SeconN = ".print_r($SeconN,true)."\n";
			echo " SearchName = ".print_r($SearchName,true)."\n";
...
function getNextID($db,$seq) {
        $result = pg_query($db, ("SELECT nextval('$seq') AS id from DUAL"));
        if ($result == FALSE)
